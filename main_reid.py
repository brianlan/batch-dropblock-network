...
def train(**kwargs):
    ...
    # dataset = data_manager.SKU(
    #     "/data2/datasets/clobotics/png/indices/head_sku-20191127/train.txt",
    #     "/data2/datasets/clobotics/png/indices/head_sku-20191127/gallery.txt",
    #     "/data2/datasets/clobotics/png/indices/head_sku-20191127/query.txt",
    # )
    # dataset = data_manager.SKU(
    #     "/tmp/png_chinadrink_train.txt",
    #     "/home/rlan/deploy/batch-dropblock-network/batch2gallery.txt",
    #     "/home/rlan/deploy/batch-dropblock-network/batch2query.txt",
    # )
    dataset = data_manager.SKU(
        opt.train_set_path,
    ...
    if opt.evaluate and opt.num_train_pids is not None:
        _num_train_pids = opt.num_train_pids
    else:
        _num_train_pids = dataset.num_train_pids

    if opt.model_name == "softmax" or opt.model_name == "softmax_triplet":
        model = ResNetBuilder(_num_train_pids, 1, True)
        ...
        model = IDE(_num_train_pids)
    elif opt.model_name == "resnet":
        model = Resnet(_num_train_pids)
    ...
    #     filename="checkpoint_ep" + str(-1) + ".pth.tar",
    # )
    ...
    for epoch in range(start_epoch, opt.max_epoch):
        if opt.adjust_lr:
            adjust_lr(optimizer, epoch + 1, init_lr=opt.lr)
        ...
        if opt.eval_step > 0 and (epoch + 1) % opt.eval_step == 0 or (epoch + 1) == opt.max_epoch:
            ...
            save_checkpoint(
                {"state_dict": state_dict, "epoch": epoch + 1},
                is_best=is_best,
                save_dir=opt.save_dir,
...
def test(model, queryloader):
    ...
    rank1 = 100.0 * correct / len(queryloader.dataset)
